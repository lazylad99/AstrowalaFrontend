...
export default function CourseInformationForm() {
  ...
  const isFormUpdated = () => {
    ...
    if (
      ...
      currentValues.courseImage !== course.thumbnailPath
    ) {
      return true;
    }
    return false;
  ...
  const onSubmit = async (data) => {
    if (editCourse) {
      if (isFormUpdated()) {
        ...
        if (currentValues.courseImage !== course.thumbnailPath) {
  ...
  return (
    <form
      ...
      className="space-y-8 rounded-md border-[1px] border-richwhite-700 bg-newBlue p-6 "
    >
